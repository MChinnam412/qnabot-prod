...
class MySQLConnection:
    ...
    def __init__(self):
        ...
        try:
            self.mydb = mysql.connector.connect(
                host=config.get("MYSQL_HOST", str, None),
                port=config.get("MYSQL_PORT", int, None),
                database=config.get("MYSQL_DB", str, None),
                user=config.get("MYSQL_USER", str, None),
                password=config.get("MYSQL_PASSWORD", str, None)
            )
            self.table_name = config.get("MYSQL_TABLE_NAME", str, None)
    ...
    def get_conversation_data(self, unique_id):
        ...
        """
        try:
            # query_table = self.cursor.execute(f"select conversation from chat_test_1 where sessionId = '{unique_id}'")
